...
public class EmailNotificationSender implements NotificationSender {
    ...
    public void send(ScheduleNotification notification) {
        for (Contato contato : notification.getRecipients()) {
            String to = contato.getEmail().getValue();
            String body;
            String subject;
            ...
            sendEmail(to, subject, body, notification.getScheduleId());
        }
    }

    private void sendEmail(String to, String subject, String body, Long scheduleId) {
        try {
            ...
            log.info("Email enviado para {} | scheduleId={}", to, scheduleId);
    ...
    private String buildVisitTemplate(VisitNotificationEmail model) {
        try {
            ...
            if (resource == null) throw new FileNotFoundException("Template não encontrado: visitNotificationTemplate.html");
    ...
    private String buildNoteTemplate(NoteNotificationEmail model) {
        try {
            ...
            if (resource == null) throw new FileNotFoundException("Template não encontrado: noteNotificationTemplate.html");
            String template = new String(resource.readAllBytes(), StandardCharsets.UTF_8);
            resource.close();
            return template
